...
class Block:
    ...
    def __init__(self, transactions, pred_digest: int, block_number: int,
                block_miner_index: int, timestamp, nonce: int):
    ...
    def __str__(self):
        ...
        if j != len(self.transactions) - 1:
            rv += ","
        rv += ("]," + str(self.block_number) + "," + str(self.block_miner_index) +
