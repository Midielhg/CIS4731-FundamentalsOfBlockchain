...
generals = []
shared_inbox = []
total_messages = 0 #counter for the total number of messages exchanged
...
def send_message(message: Message):    # adds the message to a random index of the shared_inbox
    global lock
    lock.acquire()
    shared_inbox.insert(randrange(0, len(shared_inbox) + 1), message)
    #increase total number of messages exchanged + 1
...
def verify_signatures(recepient: int, message: Message) -> bool:
    ...
    for (sig, public_key) in message.sig_set:
        if public_key not in generals_public_keys:
            thread_safe_print("General #" + str(recepient) + ": Verification failed: Reason: public_key " + str(public_key)[:7] + "... is unknown!")
            return False
        if not rsa_signature_verify(message.text, sig, public_key[1],public_key[0]):thread_safe_print("General #" + str(recepient) +": Verification failed. Reason: signature " + str(sig)[:7] + "... is not from " +str(public_key)[:7] + "..." + str(public_key)[-7:] + " on text " + message.text)
        return False
    return True
...
def general_run(index: int):
    ...
    if not index:  # the commander
        sig_set = [(rsa_message_sign("attack", generals[0].d, generals[0].n), public_key)]
        content = ("attack", sig_set)
        thread_safe_print("The commander is broadcasting the attack message...")
        if not generals[0].byzantine:  # if commander is not byzantine
            ...
            sig_set = [(rsa_message_sign("retreat", generals[0].d, generals[0].n), public_key)]
            content2 = ("retreat", sig_set)
            send_to_group(False, 0, content, 1, no_of_generals // 2 + 1)  # command half of the generals to attack
            send_to_group(False, 0, content2, no_of_generals // 2 + 1, no_of_generals)  # command the other half to retreat
    else:
        output = None
        first = True
        while True:
            ...
            if verify_signatures(index, message):
                ...
                if generals[index].byzantine:  # flip the message
                    ...
                    signature_2 = rsa_message_sign(text, generals[index].d, generals[index].n)
                    sig_set.append((signature_2, public_key))
                    content = (text, sig_set)
                    content_2 = ("retreat" if text == "attack" else "attack", sig_set)  # conflicting message
                    if first:
                        output = message.text
                        first = False
                    elif output != message.text:
                        output = None
                    thread_safe_print(f"General #{index} is relaying conflicting messages: {message.text} and {content_2[0]}")
                    send_to_all(True, index, content)
                    send_to_all(True, index, content_2)
                else:  # don't change the message
                    text = message.text
                    signature = rsa_message_sign(text, generals[index].d, generals[index].n)
                    ...
                    if first:
                        ...
                        output = None
                    thread_safe_print(f"General #{index} is relaying the message {message.text} by sending {text}")
                    send_to_all(True, index, content)
        sleep(1)  # wait before consensus
        if not generals[index].byzantine:
            thread_safe_print(f"Honest general {index}'s output: {output}")
        else:
            thread_safe_print(f"Byzantine general {index}'s output doesn't matter!")



for k in range(no_of_generals):
    public_key, private_key = rsa.newkeys(rsa_size)
    general = General(k, private_key.d, (public_key.n, public_key.e), random() > (1 - byzantine_prob))
    generals.append(general)

Thread(target=general_run,args=(0, )).start()    # commander's thread starts early
threads = []
...
print("Number of exchanged messages: " + str(total_messages))