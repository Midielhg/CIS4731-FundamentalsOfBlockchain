...
def general_run(index: int):
    ...
    if not index:    # the commander
        ...
        thread_safe_print("The commander " + " is broadcastng the attack message...")
        if not generals[0].byzantine:    #if commander is not byzantine
            ...
            send_to_group(False, 0, content2, no_of_generals // 2 + 1, no_of_generals)
        ...
        while True:
            ...
            if verify_signatures(index, message):
                ...
                if first:
                    ...
                    output = None
                thread_safe_print("General #" + str(index) + " is relaying the message " + message.text + " by sending " + text)
                send_to_all(True, index, content)
        sleep(1)    # wait before consensus
        if not generals[index].byzantine:
            thread_safe_print("Honest general " + str(index) + "'s output: " + str(output))
        else:
            thread_safe_print("Byzantine general " + str(index) + "'s output doesn't matter!")
